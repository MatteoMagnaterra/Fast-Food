...
class VisualizzaBevande1(QtWidgets.QWidget):
    def __init__(self, cart_products, order_button):
        super().__init__()
        self.setWindowTitle("Fast-Food - Visualizza Bevande")
        self.setGeometry(150, 150, 700, 600)
        self.cart_products = cart_products  # Riferimento al carrello condiviso
    ...
    def init_ui(self):
        ...
        # Titolo
        title_label = QtWidgets.QLabel("Visualizza bevande", self)
        ...
        # Caricamento delle bevande dal magazzino
        bevande = Prodotto.leggi_bevande_da_magazzino("Magazzino.json")
        for bevanda in bevande:
            product_widget = self.create_product_widget(
                bevanda.nome, bevanda.ingredienti, bevanda.prezzo, bevanda.punti
            )
    ...
    def create_product_widget(self, name, ingredients, price, points):
        ...
        # Sezione immagine e nome bevanda
...
if __name__ == "__main__":
    ...
    window = VisualizzaBevande1(cart_products, order_button)
    window.show()
    sys.exit(app.exec())