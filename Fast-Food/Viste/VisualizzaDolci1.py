...
class VisualizzaDolci1(QtWidgets.QWidget):
    def __init__(self, cart_products, order_button):
        super().__init__()
        self.setWindowTitle("Fast-Food - Visualizza Dolci")
    ...
    def init_ui(self):
        ...
        # Titolo
        title_label = QtWidgets.QLabel("Visualizza Dolci", self)
        ...
        # Caricamento dei dolci dal magazzino
        dolci = Prodotto.leggi_dolci_da_magazzino("Magazzino.json")
        for dolce in dolci:
            product_widget = self.create_product_widget(
                dolce.nome, dolce.ingredienti, dolce.prezzo, dolce.punti
            )
...
if __name__ == "__main__":
    ...
    cart_products = []
    order_button = QtWidgets.QPushButton("Ordine(0)")
    window = VisualizzaDolci1(cart_products, order_button)
    window.show()
    sys.exit(app.exec())